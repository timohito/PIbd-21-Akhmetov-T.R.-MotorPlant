	...
	public class EngineStorage : IEngineStorage
	{
		...
		private EngineViewModel CreateModel(Engine Engine)
		{
			return new EngineViewModel
			{
				...
				Price = Engine.Price,
				EngineComponents = Engine.EngineComponents.ToDictionary(recPC => recPC.Key, recPC => (source.Components.FirstOrDefault(recC => recC.Id == recPC.Key)?.ComponentName, recPC.Value))
			};
		}
	}
}
